			...
			if(hashTable[index].next == NULL) {//아무런 값도 존재하지 않음->첫번째 위치에 새 엔트리를 생성후 oldest에 값을 넣는다
                ...
                //이미 프레임이 차있을 경우
                    ...
                    if(search3->next == NULL){
                        ...
                        if (before2->next == NULL ) { //가장 마지막에 존재하므로 그 전 리스트를 종점으로 하고 삭제
					...
					oldestFrame = oldestFrame->lruRight;
                        ...
                        if(search2->next == NULL){
							...
							printf("유일한 엔트리 제거2\n");}
                                ...
                                printf("ㅇㅇㅇ\n");
                                // if((search3->next ==NULL) && (before2->next !=NULL)) {
								printf("2\n");
                                before->next = NULL;
								free(search2);
								if(DEBUG) {
                                ...
                                printf("else\n");
                                if (before->next == NULL ) { //가장 마지막에 존재하므로 그 전 리스트를 종점으로 하고 삭제
