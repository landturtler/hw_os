		...
		for (i = 0; i < numProcess; i++) {
			...
			if (procTable[i].firstLevelPageTable[firstaddr].secondLevelPageTable[secondaddr].valid == '0') {//pagefault
				...
				if (oldestFrame->pid != -1) {//안에 이미 있음
					...
					int oldSecondAddr = (oldestFrame->virtualPageNumber << firstLevelBits) >> (firstLevelBits );
...
void IPHT(struct procEntry* procTable,struct framePage* physicalMemory) {
	...
	while (1) {
		for (i = 0; i < numProcess; i++) {
			...
			unsigned index = (vpn + i) % nFrame;//프로세스i의 인덱스
			procTable[i].ntraces++;
			if(DEBUG){
     			printf("주소 읽어오기 성공\n");
    		}
			if(hashTable[index].next != NULL) {
				...
				if(search ==NULL) {
					if(DEBUG){
						printf("fault\n"); //엔트리가 존재하지 않으므로 pagefault발생, 새로운 값을 가장 앞 리스트로 넣고 lru최신화
					}
					//entry not found
					//printf("aaa\n");
					procTable[i].numPageFault++;
				...
				//==================================//이 부분을 잘 모르겟음
					/*struct invertedPageTableEntry* oldfirst = (struct invertedPageTableEntry *)malloc(sizeof(struct invertedPageTableEntry));
					oldfirst->pid = hashTable[index].next->pid;// = /원래 테이블의 가장 첫번째 엔트리
					oldfirst->virtualPageNumber = hashTable[index].next->virtualPageNumber;
					oldfirst->next = hashTable[index].next->next;
				struct invertedPageTableEntry* add2 = (struct invertedPageTableEntry *)malloc(sizeof(struct invertedPageTableEntry));
					hashTable[index].next = add2;
                    add2->pid = i;
					add2->virtualPageNumber = vpn;
					add2->frameNumber = oldestFrame->number;
					add2->next = oldfirst;//새로운엔트리에 값을 넣음 .이제 LRU최신화 작업을 한다
					*/
                ...
                *makeNext = (struct invertedPageTableEntry *)malloc(1 * sizeof(struct invertedPageTableEntry));
                ...
                (*makeNext)->next = NULL;
					if (oldestFrame->pid != -1) {//oldest에 이미 값이 존재하면 그 값제거후 다시 넣는다.
						...
						before = NULL;
						while(!(search2->virtualPageNumber == oldvpn && search2->pid == oldPid)) {
							before = search2;
						...
						//else { //search2가 중간에 예전 값을 찾았다.
							if (before = NULL) {//첫번째 엔트리에 존재한다
								if(search2->next !=NULL) { //뒤에 무언가가 더 있으면
								hashTable[oldindex].next = search2->next;
								}
								 free(search2);
							} //가장 첫번째 리스트에 존재하므로 그것만 삭제
							else if (search2->next == NULL) { //가장 마지막에 존재하므로 그 전 리스트를 종점으로 하고 삭제
								free(search2);
								before->next = NULL;
								//printf("2\n");
							}
...
//======================f=================================
			...
			else {//아무런 값도 존재하지 않음->첫번째 위치에 새 엔트리를 생성후 oldest에 값을 
				if(DEBUG){
				...
				add->frameNumber = oldestFrame->number;
				if(DEBUG) {//오른쪽 framenumber는 oldest의 fr amenumber
				printf("framenumber값 넣기 성공\n");
				}	
				add->next = NULL;// 이제 oldestFrame에 값을 넣는다.
				if(DEBUG){
        			printf("새로운거 노드 추가 성공\n");
    			}
				if (oldestFrame->pid != -1) {//oldest에 이미 값이 존재하면 그 값제거후 다시 넣는다.
...
int main(int argc, char *argv[]) {
	...
	//i//nitProcTable(procTable, numProcess, argv); //프로세스테이블 초기화'
	//initPhyMem(physicalMemory, nFrame);//프로세스 테이블과 물리 메모리 만들기
