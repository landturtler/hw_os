...
void multiLevelTable(struct procEntry* procTable, struct framePage* physicalMemory, int secondLevelBits) {
	...
	while (1) {
		for (i = 0; i < numProcess; i++) {
			...
			if (procTable[i].firstLevelPageTable[firstaddr].secondLevelPageTable[secondaddr].valid == '0' && procTable[i].firstLevelPageTable[firstaddr].secondLevelPageTable[secondaddr].frameNumber == -1) 
			 {//pagefault
				...
				oldestFrame->lruLeft->lruRight = &physicalMemory[index];s
