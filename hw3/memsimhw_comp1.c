...
void multiLevelTable(procEntry* procTable, framePage* physicalMemory) {
	int i,j;
	unsigned int virtualaddr,physicalAddr, firstaddr, secondaddr,vpn,offset;
	char charr;
	...
	while(1){
		for (i = 0; i < numProcess; i++){ 
			...
			if (feof(procTable[i].tracefp)!= 0) break;
			//printf("feof조건문 통과 \n");
			procTable[i].ntraces++;
			...
			if (procTable[i].firstLevelPageTable[firstaddr].valid == '0') {//secondpagetable을 만들어야함
				...
				//printf("valid값 바꿈 \n");
				procTable[i].firstLevelPageTable[firstaddr].secondLevelPageTable =(struct pageTableEntry*)malloc(sizeof(struct pageTableEntry)*nsecondtable);
			...
			if (procTable[i].firstLevelPageTable[firstaddr].secondLevelPageTable[secondaddr].valid == '0') {//pagefault
				...
				if( oldestFrame->pid != -1){//안에 이미 있음
					...
					procTable[i].firstLevelPageTable[firstaddr].secondLevelPageTable[secondaddr].frameNumber = oldestFrame->number; 
					//printf("이부분 잘 되는지 의심 \n");
					struct framePage *temp = oldestFrame->lruRight->lruRight; 
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < numProcess; i++) {	//���⼭���� inverted
		// rewind tracefiles
	...
	void InvertedPageTable_sim()
	{
		...
		for (i = 0; i < nProcess; i++)
		{
			printf("**** %s *****\n", proc[i].traceName);
			printf("Proc %d Num of traces %d\n", i, proc[i].ntraces);
	a		printf("Proc %d Num of Inverted Hash Table Access Conflicts %d\n", i, proc[i].numIHTConflictAccess);
